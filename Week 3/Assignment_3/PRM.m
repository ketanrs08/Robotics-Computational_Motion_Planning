...
for i = 2:nsamples
    ...
    if i >=2 && i <= 5
      ...
      for j = 1 : length(index)
        if LocalPlanner(x,samples(:,index(j)))
            nedges = nedges + 1;
            edges(nedges,1) = i;
            edges(nedges,2) = index(j);
    ...
    [sorted, index] = sort(distances);
    nbr.index = index(1:k);
        for t = 1 : length(nbr.index)
            if LocalPlanner(x,samples(:,nbr.index(t)))
                nedges = nedges + 1;
                edges(nedges,1) = i;
                edges(nedges,2) = nbr.index(t);
                edge_lengths(nedges) = sorted(t);
